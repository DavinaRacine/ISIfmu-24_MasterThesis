...
# GitHub API settings
repo = ""   # Change to target repo
token = os.getenv("GITHUB_TOKEN")      # Save GitHub token in User Environment Variables
headers = {"Authorization": f"token {token}", "Accept": "application/vnd.github.v3+json"}


# Helper Functions
def clean_text(text):
    """Remove illegal characters and normalize for Excel export."""
    if not isinstance(text, str):
        return text
    # Normalize Unicode (handle combining chars safely)
    text = unicodedata.normalize("NFKD", text)
    # Remove ASCII control characters
    text = re.sub(r"[\x00-\x1F\x7F]", "", text)
    # Remove problematic Unicode chars Excel doesn't like
    text = re.sub(r"[\u200B-\u200F\u202A-\u202E\u2060-\u206F\uFEFF]", "", text)
    # Remove stray XML characters (illegal for Excel’s XML parser)
    text = re.sub(r"[\x0B\x0C]", "", text)
    return text

def truncate_excel_text(text):
    """Truncate long text fields to avoid Excel cell length limits."""
    MAX_EXCEL_LEN = 32000  # Excel limit ~32,767 chars per cell
    if isinstance(text, str) and len(text) > MAX_EXCEL_LEN:
        return text[:MAX_EXCEL_LEN] + " [TRUNCATED]"
    return text
...
# Clean and truncate text safely before export
df = df.map(lambda x: truncate_excel_text(clean_text(x)))
